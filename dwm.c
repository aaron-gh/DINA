 ...
 *
 * The event handlers of DINA are organized in an array which is accessed
...
spawn(const Arg *arg)
{
	...
	if (fork() == 0) {
		...
		die("DINA: execvp '%s' failed:", ((char **)arg->v)[0]);
	}
}
